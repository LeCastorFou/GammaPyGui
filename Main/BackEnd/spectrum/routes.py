    ...
    try:
        ...
        if pathConf:
            ...
            if form.validate_on_submit():
                ...
                """
                if request.form['modelfit'] == '0':
                    pwl = PowerLawSpectralModel(index=form.index.data, amplitude=form.amplitude.data+" cm-2 s-1 TeV-1", reference=str(form.reference.data)+" TeV")
                elif request.form['modelfit'] == '1':
                    pwl = BrokenPowerLawSpectralModel(index1=form.index.data, index2=form.index2.data,  amplitude=form.amplitude.data+" cm-2 s-1 TeV-1", ebreak=str(form.ebreak.data)+' 1 TeV')
                elif request.form['modelfit'] == '2':
                    pwl = LogParabolaSpectralModel(amplitude=form.amplitude.data+" cm-2 s-1 TeV-1", reference=str(form.reference.data)+" TeV",alpha=form.alpha.data,beta=form.beta.data)
                elif request.form['modelfit'] == '3':
                    pwl = ExpCutoffPowerLawSpectralModel(index=form.index.data, amplitude=form.amplitude.data+" cm-2 s-1 TeV-1", reference=str(form.reference.data)+" TeV",lambda_=str(form.lambdat.data)+" TeV-1",alpha=form.alpha.data)
                else:
                    pwl = PowerLawSpectralModel(index=2, amplitude="1e-12 cm-2 s-1 TeV-1", reference="1 TeV")

                model = SkyModel(spectral_model=pwl, name="myskymod")
